 ...
 * @param {Function} onSocialLinkClick - 社交链接点击事件
 * @param {string} className - 额外的CSS类名
 */
const ArtistSidebar = ({ artist, className = '' }) => {
  // 调试日志：检查 artist 数据
  console.log('[ArtistSidebar] Artist data:', {
    artistId: artist?.id,
    ...
    <div className={`lg:w-80 flex-shrink-0 ${className}`}>
      <div className='pb-8'>
        ...
        {artist.socialLinks.otherLinks &&
          ...
          artist.socialLinks.otherLinks.some(link => isValidUrl(link.url)) && (
            <div className='mt-4'>
